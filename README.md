...
- **Ships the pre-cloned Italian voice `jarvis1`**: the image seeds the
  registry (`/data/cache/voices`) from the repo `voices/` dir, so a fresh
  container already lists `jarvis1` in `GET /v1/voices` — no runtime clone
  needed. To serve it, run with the matching model language:

  ```sh
  docker run --rm -p 8080:8000 -e POCKET_TTS_LANGUAGE=italian_24l \
    -v tts-data:/data pocket-tts-openai
  # GET  http://localhost:8080/v1/voices     -> lists "jarvis1" (custom, it)
  # POST http://localhost:8080/v1/audio/speech \
  #      -d '{"model":"tts-1","voice":"jarvis1","input":"Ciao"}'
  ```
  ...
  `/data/hf`). The voice file itself is our own cloned state (~68 MB, from 5
  joined corpus clips) and is always in the image — no token needed for the
  voice, only for the model weights.
