...
- **Model:** download `ggml-small.bin` (~466 MB) from
  ...
  `STTS_STT_MODEL_DIR` (default `{cache_dir}/stt-models`) — same
...
- New module `src/pocket_tts_openai/stt.py`:
  ...
  - When `stt_enabled=false` (or the binary/model is unavailable) the sidecar
    ...
    unaffected.
- **Port:** fixed internal `STTS_STT_PORT` (default `8787`) bound to
  `127.0.0.1` only (never exposed; the app proxies). The binary path is
  overridable (`STTS_STT_BIN`) so tests can point at a stub and the
...
| Env | Default | Meaning |
| --- | --- | --- |
| `STTS_STT_ENABLED` | `false` | Master switch; default **off** until a sidecar+binary is present (safe for existing TTS-only deploys). Set `true` to enable STT. |
| `STTS_STT_MODEL` | `small` | GGUF model name → `ggml-{model}.bin` (multilingual default). Quantized variants (`small.q5_0`) selectable by full filename. |
| `STTS_STT_MODEL_REPO` | `ggerganov/whisper.cpp` | HF repo hosting `ggml-*.bin`. |
| `STTS_STT_MODEL_DIR` | `{cache_dir}/stt-models` | Where the GGUF is cached (persisted under `/data`). |
| `STTS_STT_BIN` | `whisper-server` | Sidecar binary path (tests/advanced override). |
| `STTS_STT_HOST` | `127.0.0.1` | Internal bind (loopback only). |
| `STTS_STT_PORT` | `8787` | Internal HTTP port proxied by the app. |
| `STTS_STT_THREADS` | `4` | `-t` compute threads for the sidecar. |
| `STTS_STT_LANGUAGE` | `""` | Optional default language; empty = auto-detect. |
| `STTS_STT_IDLE_UNLOAD_S` | `300` | Kill the sidecar (reclaim its RAM) after this long without an STT request; `0` disables (independent of `STTS_IDLE_UNLOAD_S`). |
| `STTS_STT_IDLE_POLL_S` | `30` | Dedicated STT watchdog cadence (only when idle-unload `> 0`). |
...
- **Config (separate knob, independent of TTS):**
  - `STTS_STT_IDLE_UNLOAD_S` (default `300`, `0` disables)
  - `STTS_STT_IDLE_POLL_S` (default `30`)
...
- Config parse tests for every `STTS_STT_*` knob (+ `ENABLED` off/on,
...
- Config table: document the `STTS_STT_*` knobs (incl. idle eviction).
- New "Speech-To-Text" section: quickstart curl for `transcriptions`
  ...
  is evicted after `STTS_STT_IDLE_UNLOAD_S` of STT inactivity (next
  request restarts it).
...
3. Launch: `whisper-server … --ov-e-device CPU` (x86 CPU; Intel iGPU/dGPU also
   ...
   reuse it.
4. Wire `STTS_STT_OPENVINO_DEVICE` config → `--ov-e-device`; keep native
...
- **Idle-eviction of the sidecar (formerly out-of-scope, now §6):** only
  ...
  dedicated watchdog + separate `STTS_STT_IDLE_UNLOAD_S` knob and a
...
> process-kill eviction with separate `STTS_STT_IDLE_UNLOAD_S` + dedicated
